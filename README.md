...
- Set any publication link value to `"coming-soon"` (for example `pdf: "coming-soon"`) to render a disabled link chip with a SOON tag until the real URL is ready. In the publications list, clicking a card expands it to show the review pipeline (Draft, In review, Accepted, Published) with the current stage highlighted. The Sky view toggle shows the same record as fireworks over the skyline by year: clicking the sky fires a rocket, and hitting a paper opens its card.
