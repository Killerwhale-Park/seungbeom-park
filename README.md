...
- The Seoul skyline scene (hero and Sky view backdrop) is generated art. To change it, edit `scripts/skyline-gen.mjs` and run `node scripts/skyline-gen.mjs`, which rewrites `src/components/effects/skylineScene.ts`.
...
- Optional publication fields: `bibtex` adds a copy-to-clipboard button to the card, `abstract` shows when a card is expanded.
- Set any publication link value to `"coming-soon"` (for example `pdf: "coming-soon"`) to render a disabled link chip with a SOON tag until the real URL is ready.
- An experience entry with a `url` renders its organization as a link on the CV. In the publications list, clicking a card expands it to show the review pipeline (Draft, In review, Accepted, Published) with the current stage highlighted. The Sky view toggle shows the same record as fireworks over the skyline by year: clicking the sky fires a rocket, and hitting a paper opens its card.
